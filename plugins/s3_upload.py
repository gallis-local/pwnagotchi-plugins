...
class PwnS3Upload(plugins.Plugin):
    ...
    def collect_pending_uploads(self):
        ...
        for record in uploaded_records:
            ...
            if record.get('path') != normalized_path:
        ...
        for root, dirs, files in os.walk(handshakes_dir):
            ...
            for file_name in files:
                ...
                existing_record = records_by_path.get(relative_path)
                if existing_record and checksum:
                    existing_checksum = existing_record.get('checksum')

                    if existing_checksum == checksum:
                        if (
                            existing_record.get('size') != size
                            or existing_record.get('mtime_ns') != mtime_ns
                        ):
                            existing_record['size'] = size
                            existing_record['mtime_ns'] = mtime_ns
                            existing_record['last_seen'] = self.get_current_datetime()
                            records_updated = True

                        self.LogDebug(
                            f"Skipping {relative_path} - checksum unchanged (already uploaded)"
                        )
                        continue
                    ...
                    if not existing_checksum:
                        ...
                        records_updated = True
                        self.LogDebug(
                            f"Skipping {relative_path} - recorded checksum for previously uploaded file"
                        )
                        continue
